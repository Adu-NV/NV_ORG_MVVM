...
extension JobsDetailViewController : UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == 0{
            ...
            cell.jobApplyButton.isHidden = true
            
            cell.jobApplyButton.addTarget(self, action: #selector(applyNowButtonTapped), for: .touchUpInside)
...
//            cell.jobDescriptionLabel.text = (jobDetail?.data?.job_details![0].job_brief)!
    ...
    @objc func applyNowButtonTapped(){
       debugPrint("\((jobDetail?.data?.job_details![0].job_vacancy_company_name))")
    }
